...
import { ExternalLink, Globe, Clock, ChevronLeft, ChevronRight, Search } from 'lucide-react';
...
function SearchResultCarouselCard({ result, index }: { result: SearchResult; index: number }) {
  ...
  return (
    <Card 
      className={`flex-shrink-0 w-80 ${getCardHeight()} group hover:shadow-xl transition-all duration-300 border-0 bg-gradient-to-br from-white to-gray-50/50 dark:from-gray-900 dark:to-gray-800/50 cursor-pointer overflow-hidden`}
      ...
      <CardContent className="p-0 h-full flex flex-col">
        ...
        {(result.thumbnail || result.type === 'image') && !imageError && (
          <div className="relative h-48 w-full overflow-hidden bg-gray-100 dark:bg-gray-800">
            <img
              src={result.thumbnail?.src || (result.type === 'image' ? result.url : '')}
              alt={result.title}
            ...
            <div className="absolute bottom-0 left-0 right-0 p-3">
              <div className="flex items-center justify-between mb-2">
                <Badge className={`text-xs px-2 py-1 ${getTypeColor()}`}>
                  {getTypeIcon()} {result.type.charAt(0).toUpperCase() + result.type.slice(1)}
                </Badge>
                {result.age && (
                  <Badge variant="secondary" className="text-xs px-2 py-1 flex items-center gap-1 bg-black/20 text-white border-0">
                    <Clock size={10} />
                    {result.age}
                  </Badge>
                )}
        ...
        {/* Content Section */}
        <div className={`p-4 flex-1 flex flex-col ${(result.thumbnail || result.type === 'image') && !imageError ? '' : 'justify-center'}`}>
      ...
      <div className="relative">
        ...
        >
          {data.results.map((result, index) => (
            <SearchResultCarouselCard key={`${result.url}-${index}`} result={result} index={index} />
          ))}
        </div>
      </div>
